...
impl<'a> ArgumentsBuilder<'a> {
    ...
    fn parse_args(&self) -> Arguments {
        ...
        let threads : usize = self.value_of("threads").unwrap()
            .parse().unwrap();
    ...
    fn proxy(&self) -> Option<Proxy> {
        return Some(Proxy::all(
            self.value_of("proxy")?
         ).unwrap());
    }

    fn timeout(&self) -> Duration {
        let timeout_secs: usize = self.value_of("timeout").unwrap()
            .parse().unwrap();
    ...
    fn codes_verification(&self) -> CodesVerification {
        ...
        if self.is_present("invalid-codes") {
            ...
            for code in self.values_of("invalid-codes").unwrap() {
                code_list.push(
                    code.parse().expect(
                        format!("Invalid code {}", code).as_str()
                    )
                )
            }
    ...
    fn regex_verification(&self) -> Option<Regex> {
        if self.is_present("invalid-regex") {
            return  Some(
                Regex::new(
                    self.value_of("invalid-regex").unwrap()
    ...
    fn size_verification(&self) -> Option<SizeVerification> {
        ...
        if self.is_present("no-length") {
            ...
            if self.is_present("min-length") {
                min_length = self.value_of("min-length").unwrap().parse().unwrap();
            }
            else {
                min_length = 0;
            }
            ...
            if self.is_present("max-length") {
                max_length = self.value_of("max-length").unwrap().parse().unwrap();
            }
            else {
