...
impl PathDiscovererSpawner {
    ...
    fn new(
        ...
        let (
            url_sender,
            url_receiver
        ) = new_url_channel(max_paths_count);
        ...
        let paths_provider_pool = ThreadPool::with_name(
            "Providers".to_string(),
            1
        );
    ...
    fn spawn(
        ...
        let response_handlers_wait_mutexes = wait_mutex::new_vec(
            self.response_handlers_pool.max_count()
        );

        let requesters_wait_mutexes = wait_mutex::new_vec(
            self.requesters_pool.max_count()
        );
    ...
    fn spawn_response_handler(
        ...
        self.response_handlers_pool.execute( move || {
            ...
            if use_scraper {
                scraper = Box::new(
                    HtmlScraperManager::new(new_urls_sender)
                );
            } else {
...
impl PathDiscoverer {
    ...
    fn new(
        ...
        return Self {result_communicator, end_communicator}
    }
